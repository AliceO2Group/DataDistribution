...
public:
  ...
  inline
  void newDataMessages(const std::vector<fair::mq::MessagePtr> &pSrcMsgs, std::vector<fair::mq::MessagePtr> &pDstMsgs) {

    // create a new instance to support passing the same vect as in and out
    std::vector<fair::mq::MessagePtr> lNewMsgs(pSrcMsgs.size());
    lNewMsgs.clear();
