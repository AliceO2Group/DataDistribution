...
public:
  ...
  bool NumStfSendersInPartitionRequest(std::uint32_t &pNumStfSenders) {
    ...
    do {
      ...
      if (lStatus.error_code() == grpc::StatusCode::UNAVAILABLE) {
          LOG(WARNING) << "NumStfSendersInPartitionRequest: Scheduler gRPC server UNAVAILABLE. Retrying...";
  ...
  bool TfBuilderUpdate(TfBuilderUpdateMessage &pMsg) {
    ...
    LOG(ERROR) << "gRPC: TfBuilderUpdate: error code: " << lStatus.error_code() << " message: " << lStatus.error_message();
    return false;
  }
  ...
  bool StfSenderStfUpdate(StfSenderStfInfo &pMsg, SchedulerStfInfoResponse &pRet) {
    ...
    LOG(ERROR) << "gRPC: StfSenderStfUpdate: error code: " << lStatus.error_code() << " message: " << lStatus.error_message();
    return false;
  }
